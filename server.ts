...
app.post('/api/auth/login', async (req, res) => {
  ...
  // Standalone Master Admin Login
  if (
    (query === 'derby_admin' || query === 'admin' || query === 'admin@derbybet.com' || query === 'admin@derbybet.turf') &&
...
// Auto-seeding helper for default Indian Turf race centers
async function getOrSeedRaceCenters(): Promise<RaceCenter[]> {
  try {
    await ensureMongoConnected();
    let mongoCenters = await RaceCenterModel.find({}).sort({ order: 1, created_at: 1 }).lean();
    if (!mongoCenters || mongoCenters.length === 0) {
      console.log('🌱 Seeding default Indian Race Centers into MongoDB Atlas...');
...
// POST /api/admin/race-centers/seed-defaults (Restore default 9 Indian centers)
...
// POST /api/admin/race-centers (Add new race center manually)
...
app.get('/api/races/:id', async (req, res) => {
  try {
    ...
    if (mongoRace) {
      ...
      if (idx >= 0) db.races[idx] = mongoRace as any;
      else db.races.push(mongoRace as any);
...
app.get('/api/bets/my', async (req, res) => {
  ...
  try {
    ...
    if (mongoBets && mongoBets.length > 0) {
      // Sync in-memory db.bets
      mongoBets.forEach((mb: any) => {
        if (!db.bets.some(b => b.id === mb.id)) {
          db.bets.unshift(mb as any);
        }
      });
...
// Unified Admin Bootstrap endpoint (returns all admin operational data in 1 request)
app.get('/api/admin/bootstrap', async (req, res) => {
  try {
    ...
    const realUsers = safeUsers.filter((u: any) => u.role !== 'admin' && u.username !== 'admin');
    const realVolume = bets.reduce((s: number, b: any) => s + (b.stake || b.amount || 0), 0);
    const pendingBets = bets.filter((b: any) => b.status === 'PENDING');
    const openRaces = races.filter((r: any) => r.status === 'OPEN' || r.status === 'LIVE' || r.status === 'OPEN_FOR_BETTING');
    ...
    return res.json({
      ...
      deposits,
      withdrawals,
      race_centers: centers.length > 0 ? centers : undefined,
      race_days: days,
      system_settings: db.system_settings || { betting_enabled: true, max_bet_per_horse: 50000, max_win_per_race: 500000, min_bet_amount: 100, sub_admins: [] },
    });
  } catch (err: any) {
...
app.get('/api/admin/overview', async (req, res) => {
  try {
    ...
    const bets = (mongoBets && mongoBets.length > 0) ? mongoBets : (db.bets || []);
    const races = (mongoRaces && mongoRaces.length > 0) ? mongoRaces : (db.races || []);
    const realUsers = users.filter((u: any) => u.role !== 'admin' && u.username !== 'admin');
    const realVolume = bets.reduce((s: number, b: any) => s + (b.stake || b.amount || 0), 0);
    const pendingBets = bets.filter((b: any) => b.status === 'PENDING');
    const openRaces = races.filter((r: any) => r.status === 'OPEN' || r.status === 'LIVE' || r.status === 'OPEN_FOR_BETTING');
    return res.json({
...
// Admin All Bets (sorted latest placed first)
...
app.put('/api/admin/horses/:id/odds', async (req, res) => {
  ...
  try {
    ...
    if (mongoRace) {
      foundRace = mongoRace as any;
      foundHorse = foundRace!.horses.find((h: any) => 
        h.id === targetHorseId || 
        String(h.horse_no) === targetHorseId || 
        String(h.serial_no) === targetHorseId ||
        `no_${h.horse_no}` === targetHorseId ||
        `no_${h.serial_no}` === targetHorseId ||
        (h.name && h.name.toLowerCase() === targetHorseId.toLowerCase())
      ) || null;
  ...
  if (win_odds !== undefined && !isNaN(Number(win_odds)) && Number(win_odds) > 0) foundHorse.win_odds = Number(win_odds);
  if (place_odds !== undefined && !isNaN(Number(place_odds)) && Number(place_odds) > 0) foundHorse.place_odds = Number(place_odds);
...
app.post('/api/admin/races/:raceId/resume', async (req, res) => {
  ...
  for (const h of race.horses) {
    ...
    if (oddsMap) {
      ...
      if (match) {
        if (match.win_odds !== undefined && !isNaN(Number(match.win_odds))) h.win_odds = Number(match.win_odds);
        if (match.place_odds !== undefined && !isNaN(Number(match.place_odds))) h.place_odds = Number(match.place_odds);
      }
    }
  }
  saveDatabase();
...
// 5. SETTLE RACE & AUTO PAYOUT BETS (CORE REQUIREMENT - WITH DEAD HEAT SUPPORT & MULTI-FIELD MATCHING)
...
app.post('/api/admin/races/:id/settle', async (req, res) => {
  ...
  // Helper to resolve horse object from any identifier (ID, name, number)
  const findRaceHorse = (identifier: string) => {
    ...
    return race.horses.find(
      (h) =>
        ...
        String(h.horse_no) === clean ||
        String(h.serial_no) === clean
    ) || null;
  };
  ...
  const registerPlaceFactor = (idOrName: string, factor: number) => {
    if (!idOrName) return;
    ...
    if (horseObj) {
      ...
      if (horseObj.horse_no !== undefined) placeFactorMap.set(`no_${horseObj.horse_no}`, factor);
      if (horseObj.serial_no !== undefined) placeFactorMap.set(`no_${horseObj.serial_no}`, factor);
    }
  };
  ...
  if (p1.length >= 3) {
    ...
    p1.forEach(hId => registerPlaceFactor(hId, 1.0));
  ...
  if (remainingSlots > 0 && p2.length > 0) {
    if (p2.length <= remainingSlots) {
      p2.forEach(hId => registerPlaceFactor(hId, 1.0));
  ...
  if (remainingSlots > 0 && p3.length > 0) {
    if (p3.length <= remainingSlots) {
      p3.forEach(hId => registerPlaceFactor(hId, 1.0));
  ...
  // Find all bets for this race from MongoDB and memory (resilient matching)
  ...
  const betsToSettleMap = new Map<string, any>();

  allAvailableBets.forEach((b) => {
    const bNorm = norm(b.race_name);
    const isMatched = 
      b.race_id === race.id || 
      (bNorm && raceNormName && (bNorm === raceNormName || bNorm.includes(raceNormName) || raceNormName.includes(bNorm)));
  ...
  // Check whether a bet won 1st Place (WIN market)
  const isBetWinWinner = (bet: any) => {
  ...
  // Check whether a bet qualified for Top 3 (PLACE market)
  const getBetPlaceFactor = (bet: any) => {
    const betHorseName = (bet.horse_name || '').trim().toLowerCase();
    const betHorseNo = bet.horse_no !== undefined ? `no_${bet.horse_no}` : '';
    const betSerialNo = bet.serial_no !== undefined ? `no_${bet.serial_no}` : '';
    return (
      (bet.horse_id ? placeFactorMap.get(bet.horse_id) : undefined) ??
      (betHorseName ? placeFactorMap.get(betHorseName) : undefined) ??
      (betHorseNo ? placeFactorMap.get(betHorseNo) : undefined) ??
      (betSerialNo ? placeFactorMap.get(betSerialNo) : undefined) ??
      0
    );
  };
...
// Helper: Enrich Users with Financial Lifetime Totals (Unified across all endpoints)
function enrichUsersWithFinancials(
  rawUsers: any[],
  depositsList: any[],
  withdrawalsList: any[],
  betsList: any[],
...
// ⚡ Fast Unified Admin Bootstrap Endpoint (1 Request loads all admin metrics, users, bets, centers, days)
app.get('/api/admin/bootstrap', async (req, res) => {
  try {
    await ensureMongoConnected();
    ...
    const totalVolume = betsList.reduce((s: number, b: any) => s + (b.stake || b.amount || 0), 0);
    const pendingBetsCount = betsList.filter((b: any) => b.status === 'PENDING').length;
    ...
    const finalCenters = (mongoCenters && mongoCenters.length > 0) 
      ? mongoCenters 
      : ((db.race_centers && db.race_centers.length > 0) ? db.race_centers : defaultData.race_centers);
...
// 18.1 Reset Races & Keep Deposits Only
...
app.post('/api/admin/deposits/:id/approve', async (req, res) => {
  try {
    ...
    // 🔒 1. ATOMIC IDEMPOTENCY LOCK: Only allow state transition from PENDING -> APPROVED exactly ONCE!
    ...
    // If already approved in MongoDB, return early with no duplicate balance credit
    if (!reqItem) {
    ...
    // 🔒 2. ATOMIC USER BALANCE CREDIT (Only runs once!)
    ...
    // 🔒 3. RECORD TRANSACTION (Unique by reference_id & txId)
    ...
    // 🔒 4. PERSISTENT USER NOTIFICATION
    ...
    if (dIdx >= 0) db.deposit_requests[dIdx] = reqItem as any;
    else db.deposit_requests.unshift(reqItem as any);

    if (updatedUserDoc) {
      if (!db.users) db.users = [];
      const uIdx = db.users.findIndex((u) => u.id === updatedUserDoc.id);
      if (uIdx >= 0) db.users[uIdx] = updatedUserDoc as any;
      else db.users.push(updatedUserDoc as any);
    }
    if (!db.transactions) db.transactions = [];
    ...
    const nIdx = db.notifications.findIndex((n: any) => n.id === notifId || n.reference_id === reqItem!.id);
    if (nIdx >= 0) db.notifications[nIdx] = newNotif as any;
    else db.notifications.unshift(newNotif as any);

    saveDatabase();

    const userProfile = updatedUserDoc ? (({ password_hash, ...u }) => u)(updatedUserDoc as any) : undefined;
    return res.json({
      success: true,
...
// ----------------------------------------------------
app.get('/api/notifications', async (req, res) => {
  const queryUser = (req.query.user_id as string || '').trim();
  ...
  try {
    ...
    // 1. Add database notifications & index read status
    (mongoNotifs || []).forEach((n: any) => {
    ...
    // 2. Synthesize deposit notifications
    const allDeps = (mongoDeps && mongoDeps.length > 0) ? mongoDeps : (db.deposit_requests || []).filter((d: any) => userIds.includes(d.user_id) || userIds.includes(d.username));
      ...
      if (!notifMap.has(depKey) && !notifMap.has(dep.utr_number)) {
        ...
        if (dep.status === 'APPROVED') {
          notifMap.set(depKey, {
            ...
            is_read: isRead,
            created_at: dep.reviewed_at || dep.created_at || new Date().toISOString(),
          });
    ...
    // 3. Synthesize withdrawal notifications
    const allWths = (mongoWths && mongoWths.length > 0) ? mongoWths : (db.withdrawal_requests || []).filter((w: any) => userIds.includes(w.user_id) || userIds.includes(w.username));
    allWths.forEach((wth: any) => {
      ...
      if (!notifMap.has(wthKey)) {
        ...
        if (wth.status === 'SUCCESSFUL') {
          notifMap.set(wthKey, {
            ...
            reference_id: wth.id,
            is_read: isRead,
            created_at: wth.reviewed_at || wth.created_at || new Date().toISOString(),
          });
          ...
          notifMap.set(wthKey, {
            ...
            reference_id: wth.id,
            is_read: isRead,
            created_at: wth.approved_at || wth.created_at || new Date().toISOString(),
          });
        }
      }
    });
    ...
    const memNotifs = (db.notifications || []).filter((n: any) => n.user_id === queryUser);
